  ...
  class Http
    ...
    def self.with_http_error_logging(server_url, route, &cb)
      ...
      begin
        ...
        Puppet.warning("Request to #{server_url.host} on #{server_url.port} at route #{route} timed out after #{config.server_url_timeout} seconds. #{SERVER_URL_FAIL_MSG if server_url_config}")
        return e

      rescue SocketError, OpenSSL::SSL::SSLError, SystemCallError, Net::ProtocolError, IOError, Net::HTTPNotFound => e
        Puppet.warning("Error connecting to #{server_url.host} on #{server_url.port} at route #{route}, error message received was '#{e.message}'. #{SERVER_URL_FAIL_MSG if server_url_config}")
        return e

      rescue Puppet::Util::Puppetdb::InventorySearchError => e
        Puppet.warning("Could not perform inventory search from PuppetDB at #{server_url.host}:#{server_url.port}: '#{e.message}' #{SERVER_URL_FAIL_MSG if server_url_config}")
        return e
        ...
        Puppet.warning("Failed to submit '#{e.context[:command]}' command for '#{e.context[:for_whom]}' to PuppetDB at #{server_url.host}:#{server_url.port}: '#{e.message}' #{SERVER_URL_FAIL_MSG if server_url_config}")
        return e
    ...
    def self.check_http_response(response, server_url, route)
      ...
      if response.is_a? Net::HTTPServerError
        Puppet.warning("Error connecting to #{server_url.host} on #{server_url.port} at route #{route}, error message received was '#{response.message}'. #{SERVER_URL_FAIL_MSG if server_url_config}")
        :server_error
        ...
        if response.body && response.body.chars.first == "{"
          ...
          Puppet.debug("HTTP 404 (probably normal) when connecting to #{server_url.host} on #{server_url.port} at route #{route}, error message received was '#{response.message}'. #{SERVER_URL_FAIL_MSG if server_url_config}")
          :notfound
        else
