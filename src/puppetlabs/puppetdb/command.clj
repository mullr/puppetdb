...
(defmethod process-command! [(command-names :deactivate-node) 3]
  ...
  (let [certname (:certname payload)
        ...
        id  (:id annotations)
        have-newer? (fn [entity] (scf-storage/have-record-produced-after? entity certname producer_timestamp))]
    (jdbc/with-transacted-connection db
      (when-not (scf-storage/certname-exists? certname)
        (scf-storage/add-certname! certname))
      (when-not (or (have-newer? :catalogs) (have-newer? :factsets) (have-newer? :reports))
